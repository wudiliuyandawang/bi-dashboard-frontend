...
export async function getTrend(days = 7) {
  ...
  const rows = await query(sql`SELECT order_time::date AS date, COALESCE(SUM(total_price),0)::float AS amount FROM order_info WHERE order_time >= ${since} GROUP BY order_time::date ORDER BY date ASC`) as any[];
...
export async function getProductRank(limit = 10) {
  const rows = await query(sql`SELECT p.name AS product_name, SUM(o.quantity)::int AS total_quantity, SUM(o.total_price)::numeric::float AS total_sales FROM order_info o JOIN product p ON o.product_id = p.id GROUP BY o.product_id, p.name ORDER BY total_sales DESC LIMIT ${limit}`) as any[];
  return rows.map((r: any) => ({
